...
class Assign(SimpleExpression):
   ...
   def execute(self):
       try:
           self.var_container[self.var_name].value = self.rhs
       except TemVariableValueError as e:
           e.name = self.var_name
           raise e
       print_name_value(
           self.var_name,
           self.var_container[self.var_name],
           verbosity=cli.args().verbosity,
       )
...
class Query(Expression):
   ...
   def execute(self):
       variable = self.var_container[self.var_name]
       ...
       if cli.args().verbosity > 0:
           print_name_value(
               self.var_name,
               self.var_container[self.var_name],
               verbosity=cli.args().verbosity - 1,
           )
